...
**Fully built.** Filter by category + difficulty, inline search, accordion Q&A with key points and code examples. Fetches live from `GET /api/interview-questions`; falls back to static `interviewData.js` when DB is empty. Categories: `JAVA`, `ADVANCED_JAVA`, `DSA`, `SQL`, `SPRING_BOOT`, `HIBERNATE`, `AWS`. The static bank (`interviewData.js`) exports `QUESTIONS` (used as the "Import Defaults" source in the admin panel) and `CATEGORY_META` for display labels/colors. Bundled batch JSON files are served from `frontend/public/interview-batches/` (iq-batch-01 through iq-batch-06, covering Java Core, Advanced Java, DSA, SQL, AWS, Spring Boot, and Hibernate).
...
**Interview Q&A tab** — three sub-tabs: **Manage** (filterable list with ✏️ Edit + 🗑 Delete per question, "Import Defaults" button that seeds from `interviewData.js`, Delete All); **+ New Question** (single-question create/edit form); **Batch Upload** (paste raw JSON array or `{ batchName, questions: [] }` object, or load one of the 6 bundled batch files from `public/interview-batches/`). `keyPoints` is stored as a JSON string array in the DB but edited one-per-line in the form.
...
**7. Spring Boot + System Design Interview Q&A Data** *(Spring Boot partially done)*
Spring Boot questions are now live via batches iq-batch-05 and iq-batch-06 (Spring Boot Fundamentals + Spring Boot Advanced + Hibernate). Still missing from the live DB: System Design category.
...
- To add: create a new batch JSON (see iq-batch-05 format) and import via Admin → Interview Q&A → Batch Upload
